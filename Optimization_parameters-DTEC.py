...
# ---------------------导入CSV或xlsx文件--------------------------
df = pd.read_excel(r'D:\Big DP-LUE\data\53_data optimization\initial data\MF1.xlsx', engine='openpyxl')  # DataFrame
true = df['GPP (gC/m2/d)']  # 获取LUE实际值
APARsu = df['APARsu-DTEC']
APARsh = df['APARsh-DTEC']
Df = df['Df']
ts = df['Ts']  # 获取温度胁迫因子
ws = df['WEF']  # 获取水分胁迫因子
...
LUEmsu = np.arange(0.2, 0.4, 0.01)
LUEmsh = np.arange(2.1, 2.3, 0.01)
...
for su in range(len(LUEmsu)):
    ...
    for sh in range(len(LUEmsh)):
        ...
        for m in range(len(re_true)):
            GPP = (re_APARsh[m] * LUEmsh[sh] * re_Df[m] ** 1.8 + re_APARsu[m] * LUEmsu[su]) * re_ts[m] * re_ws[m]
            pred.append(GPP)
...
data.to_csv(r'D:\Big DP-LUE\data\53_data optimization\optimal results\DTEC_9-10_MF2.csv', index=False)  # 将date_ta内的结果保存成csv文件
print('处理完毕')
