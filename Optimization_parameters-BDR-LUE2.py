...
a = [0.1, 0.1**2, 0.1**3, 0.1**4, 0.1**5]
b = [0.1, 0.1**2, 0.1**3, 0.1**4, 0.1**5]
# a = np.arange(0.01, 1.1, 0.2)
# b = np.arange(0.01, 0.32, 0.05)
LUEmsh = np.arange(1.7, 1.9, 0.01)
LUEmsu = np.arange(1.0, 1.2, 0.01)
        ...
        for pa in range(len(a)):
            ...
            for pb in range(len(b)):
                ...
                for m in range(len(re_true)):
                    LCPmsu = re_PARdir[m] * LUEmsu[su] * 1/(b[pb] * re_PPFD_DIF[m] + 1)
                    LCPmsh = re_PARdif[m] * LUEmsh[sh] * re_fdPPFD[m]/(a[pa] * re_PPFD_DIR[m] + 1)
...
data.to_csv(r'D:\Big DP-LUE\data\7_data optimization\optimal results\BRS-LUE_MF3.csv', index=False)  # 将date_ta内的结果保存成csv文件
print('处理完毕')
