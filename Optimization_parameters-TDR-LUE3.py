...
a = np.arange(0.01, 0.05, 0.01)
b = np.arange(0.01, 0.05, 0.01)
LUEmsh = np.arange(4.27, 4.33, 0.01)
LUEmsu = np.arange(2.09, 2.15, 0.01)
        ...
        for pa in range(len(a)):
            ...
            for pb in range(len(b)):
                ...
                for m in range(len(re_true)):
                    PPFDsu = 1/(b[pb] * re_PPFD_DIR[m] + 1)
                    PPFDsh = re_fdPPFD[m] / (a[pa] * re_PPFD_DIF[m] + 1)
...
data.to_csv(r'D:\Big DP-LUE\data\7_data optimization\optimal results\TDR-LUE4\TDR-LUE_CRO24.csv', index=False)  # 将date_ta内的结果保存成csv文件
print('处理完毕')
