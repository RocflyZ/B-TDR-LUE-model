...
targets = ["LUEmax", "LUEcs", "Bias", "r", "MSE", "MAE", "R2", "RMSE"]  # 抬头索引
...
re_Sci = pd.Series(Sci)
re_ts = pd.Series(ts)
re_ws = pd.Series(ws)

# 创建步长为1/0.1和0.1/0.01步长的温度和LUE优化数组，pandas可处理的数据
LUEmax = np.arange(2.2, 2.6, 0.01)
LUEcs = np.arange(0.5, 0.9, 0.01)
...
for ax in range(len(LUEmax)):
    nu1 += 1
    nu2 = 0
    for cs in range(len(LUEcs)):
        nu2 += 1
        pred = []  # LUE预测值
        for m in range(len(re_true)):
            LUEbase = (LUEmax[ax] - LUEcs[cs]) * re_Sci[m] + LUEcs[cs]
            GPP = re_PAR[m] * re_fPAR[m] * LUEbase * re_ws[m] * re_ts[m]
            pred.append(GPP)
...
data = pd.DataFrame({"LUEmax": date_ta[0], "LUEcs": date_ta[1], "Bias": date_ta[2], "r": date_ta[3], "MSE": date_ta[4],
                      "MAE": date_ta[5], "R2": date_ta[6], "RMSE": date_ta[7]})

data.to_csv(r'D:\Big DP-LUE\data\7_data optimization\optimal results\CI-LUE\CI-LUE_MF2.csv', index=False)  # 将date_ta内的结果保存成csv文件
print('处理完毕')
