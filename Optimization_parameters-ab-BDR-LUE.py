...
# ---------------------导入CSV或xlsx文件--------------------------
df = pd.read_excel(r'D:\Big DP-LUE\data\7_data optimization\initial data\DBF1.xlsx', engine='openpyxl')  # DataFrame
...
a = np.arange(0.01, 1, 1)
b = np.arange(0.01, 1, 0.01)
LUEmsh = np.arange(2.56, 2.57, 0.1)
LUEmsu = np.arange(1.42, 1.43, 0.1)
...
data.to_csv(r'D:\Big DP-LUE\data\9-data_handle\ab-Bias\BDR-LUE_DBF-b.csv', index=False)  # 将date_ta内的结果保存成csv文件
print('处理完毕')
