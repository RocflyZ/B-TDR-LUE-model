...
targets = ["LUEmsu", "a", "Bias", "r", "MSE", "MAE", "R2", "RMSE"]  # 抬头索引
...
# a = [0.3, 0.2, 0.1, 0.1**2, 0.1**3, 0.1**4, 0.1**5, 0.1**6, 0.1**7, 0.1**8, 0.1**9]
a = np.arange(0.6, 1, 0.01)
LUEmax = np.arange(0.9, 1.3, 0.01)
...
for ax in range(len(LUEmax)):
    ...
    for pa in range(len(a)):
        ...
        for m in range(len(re_true)):
            ...
            PPFDsh = 1/(a[pa] * re_PPFD_DIF[m] + 1)
            GPP = (re_APARsu[m] * PPFDsu + re_APARsh[m] * PPFDsh) * LUEmax[ax] * re_ts[m] * re_ws[m]
            pred.append(GPP)
...
data = pd.DataFrame({"LUEmax": date_ta[0], "a": date_ta[1], "Bias": date_ta[2], "r": date_ta[3],
                    "MSE": date_ta[4], "MAE": date_ta[5], "R2": date_ta[6], "RMSE": date_ta[7]})

data.to_csv(r'D:\Big DP-LUE\data\7_data optimization\optimal results\RTL-LUE3\RTL-LUE_MF2.csv', index=False)  # 将date_ta内的结果保存成csv文件
print('处理完毕')
